  ...
  return (
    <header className="flex items-center h-12 px-4 gap-3 border-b border-[var(--color-border)] bg-[var(--color-surface)] flex-shrink-0 z-30">
      ...
      <div className="flex items-center gap-2 mr-2">
        <span className="font-display text-[var(--color-accent)] text-base leading-none">
          {spaceName}
        </span>
      </div>
      ...
      <div className="flex items-center gap-1 flex-1 overflow-x-auto no-scrollbar">
        {rooms.map((room) => (
          <motion.button
            ...
            className={cn(
              'flex items-center gap-1.5 h-7 px-2.5 rounded-[var(--radius-sm)] text-xs whitespace-nowrap transition-colors duration-150 cursor-pointer',
